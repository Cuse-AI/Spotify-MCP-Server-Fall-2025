...
def looks_like_lyrics(comment):
    ...
    # 3. Very long comment with no personal context
...
# Find and remove suspicious comments
removed = []
for vibe, data in t['vibes'].items():
    ...
    for song in original_songs:
        ...
        if is_lyrics:
            removed.append(f"{song.get('artist')} - {song.get('song')} ({vibe}) - {reason}")
        else:
            clean_songs.append(song)
    data['songs'] = clean_songs

print(f"Removing {len(removed)} songs with lyrics/non-personal comments:\n")
for r in removed:
    print(f"  - {r}")

# Save
json.dump(t, open('core/tapestry.json', 'w', encoding='utf-8'), indent=2, ensure_ascii=False)
print(f"\nSaved! New total: {sum(len(v.get('songs', [])) for v in t['vibes'].values())} songs")
