"""Batch 10: Introspective variations (6 sub-vibes)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch10:
    def __init__(self):
        ...
        self.search_queries = {
            'Introspective - Philosophical': ['philosophical music', 'existential songs', 'meaning of life playlist', 'deep thoughts music'],
            'Introspective - Self-Reflection': ['self reflection music', 'looking inward songs', 'soul searching playlist'],
            'Introspective - Contemplative': ['contemplative music', 'meditative songs', 'mindful playlist', 'quiet contemplation'],
            'Introspective - Life Changes': ['life changes music', 'transition songs', 'new chapter playlist', 'turning point'],
            'Introspective - Growth': ['personal growth music', 'self improvement songs', 'becoming better playlist'],
            'Introspective - Questioning': ['questioning life music', 'uncertainty songs', 'dont know playlist', 'searching answers'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 10] Introspective variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch10_introspective_variations_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 10: INTROSPECTIVE\n" + "="*70)
    scraper = RedditExpansionBatch10()
