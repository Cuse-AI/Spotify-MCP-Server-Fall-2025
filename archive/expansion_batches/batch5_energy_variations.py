"""Batch 5: Energy variations (5 sub-vibes)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch5:
    def __init__(self):
        ...
        self.search_queries = {
            'Energy - Workout': ['workout music', 'gym playlist', 'exercise motivation', 'training songs'],
            'Energy - Pump Up': ['pump up music', 'hype songs', 'get hyped playlist', 'adrenaline'],
            'Energy - Confidence': ['confidence music', 'badass songs', 'powerful playlist', 'boss music'],
            'Energy - Running': ['running music', 'cardio playlist', 'jogging songs', 'run motivation'],
            'Energy - Sports': ['sports hype', 'game day playlist', 'competition music', 'athlete music'],
        }
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch5_energy_variations_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
