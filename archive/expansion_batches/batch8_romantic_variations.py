"""Batch 8: Romantic variations (7 sub-vibes)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch8:
    def __init__(self):
        ...
        self.search_queries = {
            'Romantic - First Love': ['first love music', 'new relationship songs', 'falling in love playlist', 'butterflies feeling'],
            'Romantic - Intimate': ['intimate music', 'sensual songs', 'bedroom playlist', 'romantic night music'],
            'Romantic - Date Night': ['date night music', 'romantic dinner playlist', 'date songs', 'romance evening'],
            'Romantic - Long Distance': ['long distance love music', 'missing you songs', 'far away love', 'distance relationship'],
            'Romantic - Anniversary': ['anniversary music', 'love celebration', 'relationship milestone', 'years together'],
            'Romantic - Proposal': ['proposal music', 'asking to marry', 'engagement songs', 'will you marry me'],
            'Romantic - Slow Dance': ['slow dance music', 'first dance', 'romantic slow songs', 'couples dance'],
        }
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch8_romantic_variations_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
