"""Batch 16: Missing Emotions - Playful/Silly (Light Happy territory)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch16:
    def __init__(self):
        ...
        # PLAYFUL + SILLY variations (Light Happy territory)
        self.search_queries = {
            'Playful - Silly': ['silly songs', 'goofy music', 'funny playlist', 'playful music', 'lighthearted songs'],
            'Playful - Whimsical': ['whimsical music', 'quirky songs', 'imaginative playlist', 'fanciful music'],
            'Playful - Fun': ['fun music', 'carefree songs', 'good time playlist', 'playful upbeat'],
            'Playful - Childlike': ['childlike wonder music', 'innocent playful', 'youthful joy songs', '童心 music'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 16] Playful + Silly variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch16_playful_silly_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 16: PLAYFUL/SILLY\n" + "="*70)
    scraper = RedditExpansionBatch16()
