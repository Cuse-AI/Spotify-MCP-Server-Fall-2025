"""Batch 20: FINAL Expansion - Confident/Powerful (Self-assured energy)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch20:
    def __init__(self):
        ...
        # CONFIDENT + POWERFUL variations (Self-assured energy)
        self.search_queries = {
            'Confident - Self-Assured': ['confident music', 'self assured songs', 'swagger playlist', 'own it music', 'confident energy'],
            'Confident - Powerful': ['powerful music', 'feel strong songs', 'empowering playlist', 'badass music', 'powerful energy'],
            'Confident - Bold': ['bold music', 'fearless songs', 'daring playlist', 'brave music', 'take charge songs'],
            'Confident - Victorious': ['victorious music', 'winner songs', 'triumph playlist', 'victory music', 'champion energy'],
            'Confident - Unstoppable': ['unstoppable music', 'cant stop me', 'invincible songs', 'unbeatable playlist', 'momentum music'],
            'Confident - Boss': ['boss music', 'boss bitch energy', 'like a boss', 'im the boss playlist', 'power moves music'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 20 - FINAL] Confident + Powerful variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch20_confident_powerful_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        print("\n" + "="*70)
        print("🎉 FINAL BATCH COMPLETE! ALL EMOTIONAL ZONES COVERED! 🎉")
        print("="*70)
        return fn

def main():
    print("="*70 + "\nBATCH 20: CONFIDENT/POWERFUL - FINAL EXPANSION!\n" + "="*70)
    scraper = RedditExpansionBatch20()
