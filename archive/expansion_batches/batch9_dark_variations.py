"""Batch 9: Dark/Atmospheric variations (7 sub-vibes)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch9:
    def __init__(self):
        ...
        self.search_queries = {
            'Dark - Gothic': ['gothic music', 'goth playlist', 'dark aesthetic music', 'gothic rock'],
            'Dark - Brooding': ['brooding music', 'moody playlist', 'sullen songs', 'dark mood music'],
            'Dark - Haunting': ['haunting music', 'eerie songs', 'creepy atmospheric', 'unsettling music'],
            'Dark - Noir': ['noir music', 'detective vibes', 'mystery playlist', 'film noir music'],
            'Dark - Witchy': ['witch music', 'witchy vibes', 'occult playlist', 'dark magic aesthetic'],
            'Dark - Villain Arc': ['villain arc music', 'evil vibes', 'antagonist playlist', 'dark power'],
            'Dark - Apocalyptic': ['apocalyptic music', 'end times playlist', 'dystopian vibes', 'doom music'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 9] Dark/Atmospheric variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch9_dark_variations_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 9: DARK/ATMOSPHERIC\n" + "="*70)
    scraper = RedditExpansionBatch9()
