"""Batch 17: Missing Emotions - Chaotic/Frantic (Anxious + Energy overlap)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch17:
    def __init__(self):
        ...
        # CHAOTIC + FRANTIC variations (Anxious + Energy overlap)
        # Focus on actual chaotic genres: noise, experimental, breakcore, harsh punk, etc.
        self.search_queries = {
            'Chaotic - Frantic': ['breakcore music', 'speedcore', 'frantic drum and bass', 'fast chaotic music', 'hyperactive breakbeat', 'manic electronic'],
            'Chaotic - Overwhelming': ['noise music', 'harsh noise', 'power electronics', 'sensory overload music', 'wall of sound', 'overwhelming noise'],
            'Chaotic - Unhinged': ['harsh punk', 'grindcore', 'powerviolence', 'mathcore', 'chaotic hardcore', 'intense experimental'],
            'Chaotic - Scattered': ['glitch music', 'IDM chaotic', 'erratic electronic', 'disjointed music', 'scattered beats', 'aphex twin style'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 17] Chaotic + Frantic variations...")
        all_results = []
        for cat, queries in self.search_queries.items():
            print(f"  {cat}...", end=' ', flush=True)
            results = self.scrape_vibe_category(cat, queries, 5)  # Reduced to 5 posts
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch17_chaotic_frantic_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 17: CHAOTIC/FRANTIC\n" + "="*70)
    scraper = RedditExpansionBatch17()
