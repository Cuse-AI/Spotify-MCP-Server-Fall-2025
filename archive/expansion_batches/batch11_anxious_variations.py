"""Batch 11: Anxious/Stressed variations (6 sub-vibes)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch11:
    def __init__(self):
        ...
        self.search_queries = {
            'Anxious - Panic': ['panic attack music', 'anxiety attack songs', 'calming panic', 'when anxious'],
            'Anxious - Overwhelmed': ['overwhelmed music', 'too much going on', 'stressed out songs', 'cant handle it'],
            'Anxious - Nervous Energy': ['nervous energy music', 'jittery feeling', 'restless anxiety', 'cant sit still'],
            'Anxious - Calming Anxiety': ['music for anxiety', 'calm anxiety', 'soothing anxious', 'anxiety relief'],
            'Anxious - Existential Dread': ['existential dread', 'life anxiety', 'worried about everything', 'doom feeling'],
            'Anxious - Social Anxiety': ['social anxiety music', 'before social event', 'nervous around people', 'shy anxiety'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 11] Anxious/Stressed variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch11_anxious_variations_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 11: ANXIOUS/STRESSED\n" + "="*70)
    scraper = RedditExpansionBatch11()
