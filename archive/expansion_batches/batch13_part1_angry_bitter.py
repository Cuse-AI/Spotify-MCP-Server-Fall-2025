"""Batch 13: Missing Emotions - Part 1 (Angry + Bitter)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch13Part1:
    def __init__(self):
        ...
        # ANGRY + BITTER variations (Critical gap!)
        self.search_queries = {
            'Angry - Rage': ['rage music', 'pure anger songs', 'furious playlist', 'mad as hell music'],
            'Angry - Frustrated': ['frustration music', 'fed up songs', 'annoyed playlist', 'irritated music'],
            'Angry - Aggressive': ['aggressive music', 'violent songs', 'brutal playlist', 'raw anger'],
            'Angry - Cathartic Anger': ['cathartic anger', 'release rage', 'scream music', 'let anger out'],
            'Bitter - Resentful': ['resentful music', 'bitter songs', 'grudge playlist', 'cant let go anger'],
            'Bitter - Betrayed': ['betrayed music', 'backstabbed songs', 'betrayal playlist', 'trust broken'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 13 PART 1] Angry + Bitter variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch13_part1_angry_bitter_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 13 PART 1: ANGRY + BITTER\n" + "="*70)
    scraper = RedditExpansionBatch13Part1()
