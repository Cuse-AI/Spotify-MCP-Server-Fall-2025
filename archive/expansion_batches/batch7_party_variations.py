"""Batch 7: Party variations (6 sub-vibes)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch7:
    def __init__(self):
        ...
        self.search_queries = {
            'Party - Pregame': ['pregame music', 'getting ready party', 'pre party hype', 'before going out'],
            'Party - Club': ['club music', 'nightclub playlist', 'dance floor songs', 'clubbing music'],
            'Party - House Party': ['house party music', 'apartment party', 'small party songs', 'friend party'],
            'Party - Festival': ['festival music', 'rave playlist', 'edm festival', 'music festival vibes'],
            'Party - College': ['college party music', 'frat party songs', 'university party', 'dorm party'],
            'Party - Dance': ['dance party music', 'dancing playlist', 'dance floor bangers', 'party dancing'],
        }
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch7_party_variations_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
