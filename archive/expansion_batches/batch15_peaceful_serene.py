"""Batch 15: Missing Emotions - Peaceful/Serene (Deeper Calm exploration)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch15:
    def __init__(self):
        ...
        # PEACEFUL + SERENE variations (Deeper Calm territory)
        self.search_queries = {
            'Peaceful - Serene': ['serene music', 'peaceful songs', 'tranquil playlist', 'pure calm music'],
            'Peaceful - Meditative': ['meditation music', 'meditative songs', 'zen playlist', 'mindfulness music'],
            'Peaceful - Nature Sounds': ['nature sounds music', 'forest ambient', 'ocean waves music', 'rain sounds'],
            'Peaceful - Quiet Reflection': ['quiet reflection music', 'stillness songs', 'silent contemplation', 'peaceful solitude'],
            'Peaceful - Gentle': ['gentle music', 'soft peaceful songs', 'tender calm', 'delicate ambient'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 15] Peaceful + Serene variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch15_peaceful_serene_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 15: PEACEFUL/SERENE\n" + "="*70)
    scraper = RedditExpansionBatch15()
