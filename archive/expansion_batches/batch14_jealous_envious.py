"""Batch 14: Missing Emotions - Jealous/Envious (Anxious + Angry overlap)"""
import praw, os, pandas as pd, time, re
from datetime import datetime
from dotenv import load_dotenv
load_dotenv('../.env')

class RedditExpansionBatch14:
    def __init__(self):
        ...
        # JEALOUS + ENVIOUS variations (Anxious + Angry overlap)
        self.search_queries = {
            'Jealous - Romantic Jealousy': ['jealousy songs', 'jealous in love music', 'envy in relationships', 'possessive love songs'],
            'Jealous - Envious': ['envy songs', 'jealous of success', 'comparison music', 'wish i had what they have'],
            'Jealous - Insecure': ['insecure jealousy', 'not good enough music', 'comparison anxiety', 'jealous insecurity'],
            'Jealous - Competitive': ['competitive jealousy', 'rivalry songs', 'want to be better', 'envious ambition'],
        }
    ...
    def run_batch(self):
        print("\n[BATCH 14] Jealous + Envious variations...")
        all_results = []
    ...
    def save_for_ananki(self, data):
        ...
        fn = f'reddit_batch14_jealous_envious_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv'
        df.to_csv(fn, index=False)
        print(f"\n[OK] {fn}: {len(df)} songs")
        return fn

def main():
    print("="*70 + "\nBATCH 14: JEALOUS/ENVIOUS\n" + "="*70)
    scraper = RedditExpansionBatch14()
